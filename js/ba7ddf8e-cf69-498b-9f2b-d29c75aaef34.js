...
/* 機能別 権限マトリクス（役割×機能）。◎全操作 ○閲覧・編集 △自担当のみ 👁閲覧のみ ✕不可。
   値は記号 もしくは [記号,スコープ注記]。roleLevel もこの行から導出（NAV_FUNC で対応） */
const PERM_SYM = {
  '◎':{label:'全操作',cls:'t-green'},
  '○':{label:'閲覧・編集',cls:'t-blue'},
  '△':{label:'自担当のみ',cls:'t-teal'},
  '👁':{label:'閲覧のみ',cls:'t-amber'},
  '✕':{label:'不可',cls:'t-gray'},
};
function permCell(v){
  const sym=Array.isArray(v)?v[0]:v, scope=Array.isArray(v)?v[1]:'';
  const s=PERM_SYM[sym]||PERM_SYM['✕'];
  return `<span class="tag ${s.cls} nodot" title="${s.label}" style="font-size:13px;min-width:22px;justify-content:center">${sym}</span>${scope?`<span class="subtle" style="display:block;font-size:9.5px;margin-top:2px;line-height:1.2">${scope}</span>`:''}`;
}
const PERM_FUNCS = [
  '経営ダッシュボード・BI分析','顧客・店舗 閲覧','顧客・店舗 登録・編集','CSV一括登録',
  '請求先・宛先設定','取引履歴 閲覧','契約管理','営業活動（訪問・商談・見積）',
  'AI日報 作成・提出','AI日報 承認・差し戻し','作業計画・配車','作業実行・完了報告',
  '作業報告書（PDF）','請求管理（発行）','入金・消込','売上管理',
  '外部連携設定','マスタ管理','ユーザー・権限管理',
];
const PERM_MATRIX = {
  admin: ['◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎','◎'],
  mgr:   ['○','○','○','○','○','○','○','○',['○','自分'],'◎','○','👁','○','○','👁','○','✕','○','✕'],
  sales: [['👁','自部門'],['△','自担当'],['△','自担当'],'✕','✕',['△','自担当'],['△','自担当'],['△','自担当'],['○','自分'],'✕','👁','✕','👁','✕','✕',['👁','自部門'],'✕','✕','✕'],
  field: ['✕',['👁','担当店舗'],'✕','✕','✕',['👁','担当'],'✕','✕','✕','✕',['👁','自班'],['○','自作業'],['○','自分の作業分'],'✕','✕','✕','✕','✕','✕'],
  fin:   ['👁','○','○','○','○','○','👁','✕','✕','✕','✕','✕','○','◎','◎','○','👁',['△','一部'],'✕'],
};
